...
// saveRalData() - ЯВНО записване при бутон
await saveBomDataToServer(true); // showNotif = true
```

### Deployment workflow

1. Разархивиране/копиране на файловете
2. Поставяне на `bom_data.json` в папката
3. Стартиране на `start_server.bat` (Windows) или `python server.py` (Linux/Mac)
4. Отваряне на `http://localhost:8080/unified_bom_viewer.html`
5. Споделяне на `http://<IP>:8080/unified_bom_viewer.html` с мрежата

### Миграция от localStorage
**Автоматична:** При първо зареждане с мрежов режим, `loadAllUserData()` копира localStorage данни в `bomData.userStates`.

### Тестване
✅ Класификация се запазва в `bom_data.json`  
✅ Checkbox-ове се запазват в `bomData.userStates.checkboxes`  
✅ RAL данни се запазват в `bomData.userStates.checkboxes`  
✅ Reload запазва всички данни  
✅ Мрежов режим real-time споделя промени  
✅ Валидация НЕ изтрива класификация  
✅ Notification само при класификация (не при checkbox)  
✅ RAL запазване само при "Запази RAL данни"  

### Ползи
✅ **Централизирано съхранение** - един файл за всичко  
✅ **Мрежово споделяне** - реал-тайм sync между устройства  
✅ **Автоматични backup-и** - при всяко записване (премахнато)  
✅ **Лесен deployment** - копирай 3 файла + стартирай bat  
✅ **Cross-platform** - работи на Windows/Linux/Mac  
✅ **Mobile достъп** - през Wi-Fi от телефони/таблети  

---

## ✅ Обобщение: Фаза 6 приключена

Към 8 януари 2026 Фаза 6 е реализирана изцяло:
- Централизирано съхранение в `bom_data.json`
- Python HTTP сървър с CORS поддръжка
- Batch файл за лесно стартиране
- Мрежово споделяне в реално време
- Миграция от localStorage
- Deployment документация

---

## 🎯 Подобрения след Фаза 6 (Януари 2026)

### Подобрение #1: Версия и терминология

**Дата:** 12 януари 2026  
**Статус:** ✅ Завършено

#### Промени:
1. **Версия обновена** на `v2026-01-12`
2. **"обекти" → "възли"** в заглавието и UI текстове
3. **Редактируемо заглавие** (само в режим редактиране)
   - Бутон "✏️ Редактирай заглавие" до toggle-а
   - Запазва се в `bomData.userStates.customTitle`
   - Видимо за всички, редактиране само в режим редактиране
...
### Подобрение #2: QR код и пряк път при стартиране на сървъра
...
#### Промени:
1. **QR код генериране:**
   - Генерира `bom_visualizer_qr.png` файл
   - Показва ASCII QR код в терминала
   - Води към `http://<локален-IP>:8080/unified_bom_viewer.html`
   
2. **Пряк път (.url файл):**
   - Генерира `.url` файл с име на папката
   - Пример: `Try for success 5.url`
   - Може да се копира на други компютри за бърз достъп

3. **Библиотека:** `qrcode[pil]` инсталирана

#### Файлове променени:
- `server.py`

#### Как работи:
- При стартиране на `start_server.bat` се генерират автоматично:
  - QR код като PNG файл
  - QR код в терминала (ASCII art)
  - .url файл със същото име като папката
  
- QR кодът работи само в локална Wi-Fi мрежа
- .url файлът може да се копира на USB/мрежова папка

---

### Подобрение #3: Премахване на hover показване на картинки

**Дата:** 12 януари 2026  
**Статус:** ✅ Завършено

#### Промени:
1. **Деактивирано hover (onmouseover):**
   - Коментирани `onmouseover` и `onmouseout` атрибутите
   - Кодът остава за евентуално бъдещо пускане
   
2. **Премахнато подчертаване:**
   - Коментирано `text-decoration: underline`
   - По-добра видимост на долни черти (_) в имената

#### Файлове променени:
- `unified_bom_viewer.html`

#### Резултат:
- Картинки се показват **само при клик**
- Долните черти между цифрите се виждат ясно

---

### Подобрение #4: Режим "Редактиране" вместо "Админ режим"
...
#### Промени:
1. **Премахната парола:**
   - Toggle switch работи директно без modal за парола
   - Функциите `showPasswordModal()` и `validatePassword()` остават но не се използват
   
2. **Преименуване:**
   - "Админ режим" → "Редактиране"
   - "АДМИН РЕЖМ" → "РЕДАКТИРАНЕ"
   - Всички съобщения обновени
...
#### Резултат:
- По-прост UX - без парола
- По-ясна терминология - "Редактиране" е интуитивно

---

## 📋 Списък на файлове за копиране в нова директория

**Минимален комплект (5 файла/папки):**
1. ✅ `unified_bom_viewer.html` - визуализаторът
2. ✅ `bom_data.json` - BOM данни + класификация + потребителски настройки
3. ✅ `server.py` - Python сървър
4. ✅ `start_server.bat` - batch файл за стартиране
5. ✅ `Screenshots/` - цялата папка с подпапки (Parts/ и Assemblies/)

**Опционални:**
- `unified_bom_viewer_documentation.md` - документация
- `SETUP_INSTRUCTIONS.md` - deployment инструкции

**Не са нужни:**
- `.venv/` - виртуална среда
- `.git/` - Git история
- `bom_classifier.html` - стар файл
- Markdown файлове с документация

---

## 🎯 Резюме на всички подобрения

**Фаза 6.1 (12 януари 2026):**
- ✅ Версия v2026-01-12
- ✅ "възли" вместо "обекти"
- ✅ Редактируемо заглавие
- ✅ QR код при стартиране
- ✅ .url файл с име на папката
- ✅ Премахнато hover на картинки
- ✅ Премахнато подчертаване на текста
- ✅ "Редактиране" вместо "Админ режим"
- ✅ БЕЗ парола за режим редактиране

**Технически детайли:**
- QR библиотека: `qrcode[pil]`
- Функции запазени: `showPasswordModal()`, `validatePassword()` (за бъдещо използване)
- Hover код: коментиран но запазен
- Подчертаване: коментирано но запазено

---

**Край на Integration Log**
